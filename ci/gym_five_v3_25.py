...
Each story Kanto Leader keeps the six prepared mons installed by the balance
pass. At battle creation exactly five are copied into a temporary trainer party.
The omitted slot is derived from the save Trainer ID plus Gym index, so reset /
reload on the same save cannot reroll it.
...
def main() -> int:
    ...
    report = {
        ...
        "compileFix": "removed nonexistent AI_FLAG_RANDOMIZE_PARTY_INDICES reference",
    }
