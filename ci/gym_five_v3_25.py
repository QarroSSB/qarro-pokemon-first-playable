...
static bool8 TryCreateQarroFiveOfSixGymParty(struct Pokemon *party, u16 trainerNum, const struct Trainer *trainer)
{
    ...
    // The source trainer blocks are audited as exactly six prepared mons.
    if (trainer->partySize != PARTY_SIZE || trainer->overrideTrainer != TRAINER_NONE)
        return FALSE;

    // READ_OTID_FROM_SAVE is immutable within a save. Therefore the same save
    // always gets the same omitted slot for a given Gym, including after reset.
...
def main() -> int:
    ...
    print(
        f"[{MARKER}] PASS: eight Kanto Leaders keep six prepared mons and battle with "
        f"a save-stable five; all non-target trainers keep native party creation"
    )
    return 0
