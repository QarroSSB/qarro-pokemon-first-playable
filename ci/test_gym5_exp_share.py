...
Isolated to the test/gym5-exp-share-stable branch. Runs after the normal v3.2
balance pass, removes the first (lowest-priority) member from each six-Pokemon
Kanto leader roster while preserving the ace as the final member, then forces
the Expansion's native Gen VI party Exp Share path on for the whole party.
...
def trim_party(text: str, trainer: str) -> str:
    ...
    if len(species_matches) != 6:
        raise RuntimeError(f"{trainer}: expected exactly 6 Species entries, got {len(species_matches)}")
...
def main() -> int:
    ...
    print("[QARRO_TEST_GYM5_EXP_SHARE] PASS: all 8 Kanto gyms use 5 Pokemon; native party-wide Exp Share forced ON via " + str(exp_path.relative_to(root)))
    return 0
